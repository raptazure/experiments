...
//
// It uses Option to represent nullability in safe Rust. It has zero overhead over a null pointer due to the NonZero optimization.
// It uses an Rc (Reference Counted) pointer to give ownership of the next node to the current node. And a Weak (weak Reference Counted) pointer to reference the previous node without owning it.
